    ...
    def _validate(self, epoch_idx):
        ...
        with torch.set_grad_enabled(enable_grad):
            ...
            for batch_idx, batch in enumerate(zip(*loader)):
                ...
                calc_begin = time()
                output, acc, actual_labels, predicted_labels = self.model([elem for each_batch in batch for elem in each_batch])
                ...
                save_labels_path = os.path.join(self.result_path, "predicted_labels.txt")  # 选择你希望的文件路径
                with open(save_labels_path, "a+") as output_file:
                    for actual_label, predicted_label in zip(actual_labels, predicted_labels):
                        output_file.write(f"Actual Label: {actual_label}, Predicted Label: {predicted_label}\n")
